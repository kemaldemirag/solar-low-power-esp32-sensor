...
Per the portfolio workflow governance report (P01-04), re-scoped under the 17.09.2026 REFERENCE_BASELINE. Defines the behavioral contract the firmware and hardware design must satisfy for the sensor I2C interface. The sensor is now selected (Bosch BME280, `06_decisions/decision_register.md` DEC-06); voltage domain, address, stabilization delay and pull-up sizing are closed (§2, §3, §6). **2026-09-19 (batch 7):** the §6 hardware-safety finding (DEC-12, I2C pin logic-high while VDDIO is off) is now `DECIDED (candidate)` — gate the I2C pull-up rail on the same switched `SENSOR_3V3` rail as the sensor, via TI TPS22916 — but PV-04 (I2C back-powering/off-state leakage bench measurement) remains `PHYSICAL_VALIDATION_REQUIRED` before this document is fully contract-complete for a schematic.
...
**Hardware-safety finding (DEC-12) — `DECIDED (candidate)`, 2026-09-19 batch 7:** BME280's own datasheet forbids holding SDI/SDO/SCK/CSB at logic-high while VDDIO is off — doing so can drive an overcurrent through the pin's ESD protection diode and cause permanent damage. **Resolution:** the sensor's power rail (`SENSOR_3V3`, VDD+VDDIO together) and the I2C pull-ups are gated together on the same switched rail, via a TI TPS22916 load switch (10 nA leakage, integrated output discharge, reverse blocking) — this replaces the earlier implicit assumption that pull-ups sit on a fixed, always-on rail. ESP32 forces its I2C GPIOs to safe/high-Z before `SENSOR_3V3` is switched off. **Remaining:** PV-04 (I2C back-powering / off-state leakage bench measurement) is `PHYSICAL_VALIDATION_REQUIRED` before §2/§6 are fully implementation-ready for a schematic — see `06_decisions/decision_register.md` DEC-12.
...
- **I2C pin logic-high during power-gating (DEC-12): `DECIDED (candidate)`, batch 7** — TPS22916 gates the I2C pull-up rail with `SENSOR_3V3`; see §6. PV-04 (physical) still required before this contract is fully implementation-ready.
