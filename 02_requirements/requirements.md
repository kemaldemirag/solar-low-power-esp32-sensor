...
`IN_PROGRESS` above means a candidate topology/part decision now exists in `06_decisions/decision_register.md` (DEC-01..08), not that the requirement is `IMPLEMENTED` or `VERIFIED` — every underlying numeric value is still secondary-source corroborated at best (see `06_decisions/evidence_gaps.md` GAP-07) or fully open. SOL-012 is `NOT_APPLICABLE` as a candidate decision (DEC-08), not a closed one, since it depends on SOL-006's sensor selection remaining BME280.
