...
| `I_charger_quiescent` | MCP73871 quiescent draw during no-solar/no-USB (battery-only) operation | SOL-015 | REF-09 | **OPEN — GAP-08, still ambiguous after a 2026-09-17 batch-2 `WebSearch` pass.** Three supply-current states were sourced (260 µA charge-complete / 180 µA standby / 28 µA shutdown, batch 1), and a further search found a differently-named parameter, "Battery Discharge Current (Output Reverse Leakage Current) during Shutdown = 30 µA typ / 40 µA max," that by description sounds more applicable to a no-input condition — but it was not confirmed to be the same measurement point as the 28 µA figure, or a distinct one. Neither value is adopted; assuming either without that confirmation would be an unsupported guess. |
