...
| `I_charger_quiescent` | MCP73871 quiescent draw during no-solar/no-USB (battery-only) operation | SOL-015 | REF-09 | **CLOSED (GAP-08) — 30 µA typ (40 µA max).** IDISCHARGE @ VBAT=Power Out, No Load. **Batch 7 flag:** DEC-03 (MCP73871 selection) is reopened to `BLOCKED` on a self-verified conflict with the panel-target V_OC — this term's value is unchanged here (no replacement selected yet) but is provisional/at-risk pending that resolution. |
...
`power_budget_calc.py`'s `SleepCurrentBudget` now has seven fields (added `i_load_switch_leakage_ua`, batch 7). Running it with the regulator and arbitration terms unset (current actual state) correctly raises rather than silently omitting them or reusing stale figures — verified:

```python
sleep = SleepCurrentBudget(
    i_esp32_sleep_ua=10, i_divider_leakage_ua=0, i_sensor_standby_ua=0.1,
    i_charger_quiescent_ua=30, i_load_switch_leakage_ua=0.01,
    # i_regulator_iq_ua, i_arbitration_iq_ua left unset
)
sleep.total_ua()
# -> ValueError: Cannot compute sleep-current total: OPEN terms ['i_regulator_iq_ua', 'i_arbitration_iq_ua']
```
...
- SOL-003 (charger IC): **REOPENED to BLOCKED, batch 7** — DEC-03, MCP73871's own abs-max VIN conflicts with the panel-target V_OC; `I_charger_quiescent`'s current 30 µA figure is provisional/at-risk pending resolution, not yet changed.

## 4. Sleep-current total: not computable

Five of seven terms are set as of batch 7; `I_regulator_Iq` and `I_arbitration_Iq` are `OPEN`. **No total is reported** — the previous 40.125 µA figure (2026-09-17) assumed TPS7A02 remained in the design and is retracted along with the decision that produced it. Once DEC-04's part is confirmed past PV-01 (or TPS63802 substituted) and TPS2121's Iq is sourced from its own datasheet, re-run the script above with those values filled in.
...
- `I_charger_quiescent`: CLOSED numerically (30 µA), but **flagged provisional/at-risk** — DEC-03 (MCP73871) is reopened to `BLOCKED` on a self-verified Voc conflict (batch 7); a future charger replacement would require re-sourcing this term.
